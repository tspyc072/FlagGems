...
def scatter_reduce_sum_2d_kernel(
    ...
    for i in range(LOOP):
        offsets = (base_offsets + i * BLOCK).to(tl.int32)
        mask = offsets < N
        ...
        if DIM == 0:
            ...
            idx = tl.load(index_ptr + idx_offsets, mask=mask, other=0).to(tl.int32)
...
def scatter_reduce(inp, dim, index, src, reduce, *, include_self=True):
    ...
    """
    logger.debug("GEMS_ENFLAME SCATTER_REDUCE_TWO")
    ...
    # GCU300 does not support 64-bit data types; the kernel loads index as data,
    # so downcast int64 indices to int32 before the kernel launch.
...
def scatter_reduce_(inp, dim, index, src, reduce, *, include_self=True):
    ...
    logger.debug("GEMS_ENFLAME SCATTER_REDUCE_TWO_")
...
def scatter_reduce_out(inp, dim, index, src, reduce, *, include_self=True, out=None):
    ...
    logger.debug("GEMS_ENFLAME SCATTER_REDUCE_TWO_OUT")
