...
def _unique_dim_group_id_kernel(
    ...
    group_id = tl.cumsum(diff.to(tl.int32), axis=0)
...
def _unique_dim_row_hash_chunk_kernel(
    ...
    vals_i64 = vals.to(tl.int32)
    offsets_i64 = offsets.to(tl.int32)
...
def _unique_dim_inverse_permutation_kernel(
    ...
    tl.store(inverse_ptr + sorted_indices, offsets.to(tl.int32), mask=mask)
...
def _unique_dim_build_key_kernel(
    ...
    if FIRST:
        row = offsets.to(tl.int32)
    else:
        row = tl.load(indices_ptr + offsets, mask=mask, other=0)
    base = row * row_stride + col

    if REMAP_KIND == 0:  # _REMAP_INT
        x = tl.load(flat_ptr + base, mask=mask, other=0).to(tl.int32)
        val = x + KEY_OFFSET
    elif REMAP_KIND == 1:  # _REMAP_FP16
        bits = tl.load(flat_ptr + base, mask=mask, other=0).to(tl.int32) & 0xFFFF
        sign = (bits & 0x8000) != 0
        val = tl.where(sign, bits ^ 0xFFFF, bits ^ 0x8000)
    else:  # _REMAP_FP32
        bits = tl.load(flat_ptr + base, mask=mask, other=0).to(tl.int32) & 0xFFFFFFFF
...
def _build_composite_key(
    ...
    """
    out = torch.empty(num_rows, dtype=torch.int32, device=flat_view.device)
...
def _argsort_keys(keys: torch.Tensor):
    ...
    num_keys = keys.numel()
    if num_keys == 0:
        return torch.empty(0, dtype=torch.int32, device=keys.device), keys
    if num_keys <= _UNIQUE_DIM_RANK_SORT_MAX_KEYS:
        perm = torch.empty(num_keys, dtype=torch.int32, device=keys.device)
...
def _group_id_from_sorted(sorted_keys: torch.Tensor):
    ...
    if num_rows == 0:
        return torch.empty(0, dtype=torch.int32, device=device), -1
    if num_rows <= _UNIQUE_DIM_GROUP_SCAN_BLOCK_SIZE:
        group_id = torch.empty(num_rows, dtype=torch.int32, device=device)
        last_group_id = torch.empty((), dtype=torch.int32, device=device)
    ...
    diff = ((sorted_keys[1:] - sorted_keys[:-1]) != 0).to(torch.int32)
    group_id = torch.cat(
        [
...
def _lex_argsort_rows_composite(flat: torch.Tensor):
    ...
    device = flat.device
    if num_cols == 0:
        indices = torch.arange(num_rows, dtype=torch.int32, device=device)
...
def _lex_argsort_rows_cascade(flat: torch.Tensor) -> torch.Tensor:
    ...
    num_rows, num_cols = flat.shape
    indices = torch.arange(num_rows, dtype=torch.int32, device=flat.device)
...
def _unique_dim_row_hash(flat: torch.Tensor) -> torch.Tensor:
    ...
    row_hash = torch.empty(num_rows, dtype=torch.int32, device=flat.device)
...
def _unique_dim_inverse(
    ...
    inverse_indices = torch.empty(
        num_rows, dtype=torch.int32, device=sorted_indices.device
    )
    if num_rows == 0:
        return inverse_indices

    inverse_in_sorted = torch.cumsum(is_first.to(torch.int32), dim=0) - 1
    grid = (triton.cdiv(num_rows, _UNIQUE_DIM_GATHER_BLOCK_SIZE), 1, 1)
    with torch_device_fn.device(sorted_indices.device.index):
...
def _unique_dim_counts(
    ...
    counts = torch.empty(num_unique, dtype=torch.int32, device=is_first.device)
    if num_unique == 0:
        return counts
...
def unique_dim(
    ...
    """
    logger.debug("GEMS_ENFLAME UNIQUE_DIM")
    ...
    if all_unique:
        if return_counts:
            counts = torch.ones(size_dim, dtype=torch.int32, device=device)
        if return_inverse:
        ...
        # GCU300 keeps inverse_indices/counts in int64 host tensors; sorted_indices
        # is int64 here too. No 64-bit kernel load/store happens on the all_unique
        # path (no kernel runs on these tensors).
        return output, inverse_indices, counts
    ...
    # GCU300 does not support 64-bit data types. All internal index/group tensors
    # are int32 on the device (kernels load/store int32). Cast the final
    # inverse_indices/counts back to int64 to match torch.unique semantics.
    if inverse_indices is not None:
        inverse_indices = inverse_indices.to(torch.int64)
