...
def flat_radix_count_kernel(
    ...
    counts = tl.zeros((RADIX_SIZE,), dtype=tl.int32)
    if USE_HISTOGRAM:
        counts = tl.histogram(digit, RADIX_SIZE, active).to(tl.int32)
    else:
        for radix_bin in tl.static_range(0, RADIX_SIZE):
...
def flat_radix_update_kernel(
    ...
    prev = cumsum - counts
    take = (k_to_find <= cumsum) & (k_to_find > prev)
    selected_bin = tl.min(tl.where(take, bins, RADIX_SIZE - 1), axis=0).to(tl.int32)
    counts_before = tl.max(tl.where(take, prev, 0), axis=0)
...
def _full_nan_result(shape, dtype, device):
    ...
    indices = torch.zeros(shape, dtype=torch.int32, device=device)
    return NanMedian(values=values, indices=indices.to(torch.int64))
...
def _nanmedian_kthvalue_fallback(inp, M, N):
    inp = inp.reshape(M, N)
    if torch.is_floating_point(inp):
        valid_count = torch.empty((M,), dtype=torch.int32, device=inp.device)
...
def _nanmedian_dim_impl(inp, dim, keepdim, out=None, use_ascend_float_select=True):
    ...
    if inp.ndim == 0:
        if out is None:
            values = inp.clone()
            indices = torch.zeros((), dtype=torch.int32, device=inp.device)
        else:
            values, indices = out
            values.copy_(inp)
            indices.zero_()
    ...
    if N == 0:
        ...
        if out is None:
            ...
            indices = torch.empty(compute_shape, dtype=torch.int32, device=inp.device)
            if not keepdim:
    ...
    if is_cuda and inp.dtype in RADIX_SELECT_DTYPES and in_radix_range:
        ...
        state = torch.empty((M, 3), dtype=torch.int32, device=inp.device)
        num_warps = 4 if block_n <= 512 else 8
...
def _nanmedian_cuda_flat_radix_select(inp, out=None):
    ...
    state = torch.empty((3,), dtype=torch.int32, device=flat.device)
...
def _nanmedian_flat_impl(inp, out=None):
    ...
    indices = torch.empty((), dtype=torch.int32, device=inp.device)
    _nanmedian_dim_impl(
        flat,
        0,
        False,
...
def nanmedian(inp):
    logger.debug("GEMS_ENFLAME NANMEDIAN")
