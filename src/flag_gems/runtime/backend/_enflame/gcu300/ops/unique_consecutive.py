...
def simple_unique_consecutive_flat(
    ...
    inverse_indices = (
        torch.empty(num_tasks, dtype=torch.int32, device=data.device)
        if return_inverse
        else None
    )
    idx = (
    ...
    unique_size = torch.empty([1], dtype=torch.int32, device=data.device)
...
def large_unique_consecutive_flat(
    ...
    tile_sum = torch.empty(global_ctas_num, dtype=torch.int32, device=data.device)
    data_out = torch.empty_like(data)
    inverse_indices = (
        torch.empty(num_tasks, dtype=torch.int32, device=data.device)
        if return_inverse
...
def unique_consecutive(
    ...
    """
    logger.debug("GEMS_ENFLAME UNIQUE_CONSECUTIVE")
    ...
    if num_tasks == 0:
        ...
        inverse_indices = (
            torch.empty(0, dtype=torch.int32, device=input.device)
            if return_inverse
            else None
        )
        counts = (
    ...
    if inverse_indices is not None:
        # GCU300 computed inverse_indices in int32; cast back to int64 to match
        # torch.unique_consecutive semantics.
