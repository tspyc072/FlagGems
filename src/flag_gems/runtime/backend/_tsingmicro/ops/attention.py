...
def _attn_fwd_inner(
    ...
    # causal = False
        ...
        lo, hi = 0, KV_CTX
    # When Q_CTX >> KV_CTX, causal stage ranges can extend past KV_CTX.
    # Clamp so we do not run empty/partial tiles that perturb M.
    hi = min(hi, KV_CTX)
    lo = min(lo, hi)
...
def _prune_configs(configs, named_args, **kwargs):
    ...
    for cfg in configs:
        ...
        HEAD_DIM = named_args.get("HEAD_DIM")
        if BLOCK_N <= HEAD_DIM:
            pruned.append(cfg)
    return pruned
...
def _attn_bwd_dkdv(
    ...
    hd_mask = offs_k < BLOCK_DMODEL_ACTUAL
    ...
    for blk_idx in range(num_steps):
        ...
        do = tl.load(do_ptrs, mask=offs_m_mask[:, None] & hd_mask[None, :], other=0.0)
...
def scaled_dot_product_attention_backward(
    ...
    # txda torch.zeros does not accept memory_format; allocate then contiguous().
    dq = torch.zeros_like(query).contiguous()
    dk = torch.zeros(
    ...
    grid = lambda meta: (
        max(
            ...
            ),  # _attn_bwd_dkdv traverse the key-value sequence
            triton.cdiv(
                Q_CTX, meta["BLOCK_M2"]
            ),  # _attn_bwd_dq traverse the query sequence
        ),
        1,
        BATCH * Q_HEAD,
    )
    ...
    if group_head > 1:
        ...
        # Sum in fp32 to avoid bf16/fp16 accumulation drift across Q heads.
        dk = dk.float().sum(dim=2).to(key.dtype)
        dv = dv.float().sum(dim=2).to(value.dtype)
...
def flash_attn_varlen_func(
    ...
    if use_c_extension:
        ...
        if block_table is None:
            ...
            ), "cu_seqlens_k must be provided for non-paged KV"
        else:
            assert (
        ...
        padded_head_dim = triton.next_power_of_2(original_head_dim)
        original_out = out
        if padded_head_dim != original_head_dim:
            pad = padded_head_dim - original_head_dim
            q, k, v = [F.pad(x, (0, pad)) for x in (q, k, v)]
            # A user-provided output has the unpadded shape.
            out = None
        ...
        out, q, k, v, softmax_lse, *_ = mha_varlan_fwd(
            ...
            out,
            cu_seqlens_q,
            # Paged KV uses seqused_k, but mha_varlan_fwd still requires a
            # cu_seqlens_k tensor argument. Non-paged KV always uses the real
            # cumulative lengths to locate each sequence in the packed buffer.
