...
def _should_scan_duplicate_index(index, out_dim, reduce, dtype):
    # Sunrise atomicCAS cannot lower 16-bit values, and its floating-point CAS
    # loop can apply prod updates more than once even when index is unique.
    # Use the existing scan kernel for every reduction that would require CAS.
...
def index_reduce_(inp, dim, index, source, reduce, *, include_self=True):
    logger.debug("GEMS_SUNRISE INDEX_REDUCE_")
