...
@triton.jit
def cat_copy_func_kernel_single(
    out_ptr,
    in_ptr,
    dim_size_in,
    dim_size_out,
    ...
    pid_x = tl.program_id(0)
    offsets = pid_x * BLOCK_X + tl.arange(0, BLOCK_X)
...
def _cat_run_kernel(
    ...
    if dim == 0 and any(not tensor.is_contiguous() for tensor in A):
        BLOCK = 1024
        ...
        for tensor in A:
            ...
            total_elements = tensor_contig.numel()
            dim_size_in = tensor_contig.shape[dim]
            grid = lambda meta: (triton.cdiv(total_elements, meta["BLOCK_X"]),)
            cat_copy_func_kernel_single[grid](
                out,
                tensor_contig,
                dim_size_in,
                dim_size_out,
...
def cat_out(
    ...
    *,
    ...
    logger.debug("GEMS_SUNRISE CAT_OUT")
    A = list(A)
